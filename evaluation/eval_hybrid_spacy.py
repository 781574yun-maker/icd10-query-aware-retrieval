# eval_hybrid_spacy.py — Hybrid (spacy + BERT)
import pandas as pd
import numpy as np
import torch
...
def hybrid_spacy(q, k):
    ...
    tfidf_vec = vec_s.transform([q_cut])
    tfidf_scores = cosine_similarity(tfidf_vec, mat_s)[0]
...
ks = [1, 5, 10]
for k in ks:
    p = precision_at_k(hybrid_spacy, k)
    print(f"Hybrid (spacy) Precision@{k} = {p:.4f}")
